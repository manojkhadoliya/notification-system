...
/**
 * `PushGateway` port implementation that never calls a real provider —
 * lets the full pipeline (dedupe, rate limiting, retry ladder, DLQ) be
 * demoed and tested without an FCM project. Selected instead of
 * `FcmPushGateway` by whichever composition root's env config asks for
...
export class MockPushGateway implements PushGateway {
  ...
  async send(command: ChannelCommand): Promise<GatewaySendResult> {
    ...
    // failures look transient by default — same call MockSmsGateway
    // makes.
    return {
      success: false,
      error: `mock: simulated failure sending to ${payload.token}`,
      retryable: true,
    };
  }
}
