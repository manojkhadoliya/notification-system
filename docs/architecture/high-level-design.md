...
| Consistency | Eventually consistent status reads | A `GET` right after `202 Accepted` may briefly 404 until the projection catches up — accepted trade-off, see [ADR 0008](../adr/0008-elastic-scale-data-plane.md) |
...
| Channel rollout | All four channels together, one local-only phase | Gateway ports were already additive; no de-risking benefit left to phasing | [ADR 0004](../adr/0004-phased-channel-rollout.md) |
| Notification-delivery datastore | Kafka (write) + Cassandra (read), CQRS | Single-writer Postgres + outbox has a hard write-path ceiling; Kafka+Cassandra scale by adding nodes | [ADR 0008](../adr/0008-elastic-scale-data-plane.md) |
| Identity/preferences/templates datastore | Postgres, Redis-cached reads | Write volume never approaches the notification hot path; caching keeps Postgres off the dispatch-rate read path | [ADR 0003](../adr/0003-database-postgres.md) |
