 ...
 * only `CHANNEL` differs, and the "send" `DispatchService` calls is
 * `FeedWritingInAppGateway` (writes `NotificationFeedItem`, then
 * publishes to Redis pub/sub) rather than an external provider — see
 * `feed-writing-gateway.ts` and ADR 0012.
 *
...
export class WorkerService {
  ...
  async handle(message: ConsumedMessage): Promise<void> {
    ...
    try {
      ...
      console.error(
        `services/worker-inapp: failed to parse a message on ${message.topic}, skipping`,
        err,
      );
      return;
    }
    ...
    console.error(
      `services/worker-inapp: consumed an unexpected topic "${message.topic}", skipping`,
    );
  }
