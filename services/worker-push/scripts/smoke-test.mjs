#!/usr/bin/env node
// Round-trips a real command through a live services/worker-push
...
//   pnpm --filter @notification-system/worker-push start &   (reads .env — see .env.example; defaults to the mock gateway)
...
// straight onto command.push (bypassing services/router — this is a
...
async function main() {
  ...
  const kafka = createKafka({
    ...
    clientId: "worker-push-smoke-test",
  });
  ...
  const consumer = new KafkaConsumer(kafka, {
    groupId: `worker-push-smoke-test-${randomUUID()}`,
  ...
  console.log("Publishing a ChannelCommand directly onto command.push...");
  await broker.publishCommand({
    ...
    channel: "push",
    priority: "standard",
    renderedPayload: { token: "fcm-token", title: "Hi", body: "smoke test" },
    attemptNumber: 1,
  });

  console.log(
    `Waiting up to ${TIMEOUT_MS}ms for services/worker-push to dispatch and publish delivery-status...`,
  );
  ...
  console.log("\nAll services/worker-push smoke tests passed.");
