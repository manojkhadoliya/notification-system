// In-memory fakes for every port `services/worker-push` depends on
// (through `DispatchService` and directly). Used by
// `worker-service.test.ts` — same "test against fakes behind the real
// port" approach used throughout this repo. Not a `*.test.ts` file
// itself, so it compiles to plain `dist/test-support.js`.
import type {
  ChannelCommand,
  DedupeClaim,
  DedupeRepository,
...
export class FakeMessageBroker implements MessageBroker {
  ...
  async publishBroadcast(): Promise<void> {
    // Not exercised by services/worker-push — that's services/fanout-expander's job.
  }

  async publishChunk(): Promise<void> {
    // Not exercised by services/worker-push — see publishBroadcast's comment above.
  }
}
...
export class FakeNotificationRepository implements NotificationRepository {
  ...
  async findById(): Promise<NotificationRequest | null> {
    return null; // not exercised by services/worker-push
  }

  async save(): Promise<void> {
    // not exercised by services/worker-push
  }

  async findAttempts(): Promise<DeliveryAttempt[]> {
    return []; // not exercised by services/worker-push
  }
