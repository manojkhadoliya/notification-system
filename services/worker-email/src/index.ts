...
// A long retry-tier wait (up to 30 minutes for command.email.retry-30m)
// must not stall this worker's other assigned partitions — including
// fresh command.email attempt-1 messages — behind it. See
...
/**
 * Real entrypoint — wires concrete `infra-*`/`providers-email` adapters
...
async function main(): Promise<void> {
  ...
  startTracing({ serviceName: "worker-email" });
  ...
  const [
    ...
    import("@notification-system/providers-email"),
  ]);
  ...
  // No real provider to select between — providers-email currently ships
  // only MockEmailGateway (see providers-email/README.md).
  const gateway = new MockEmailGateway({
    ...(config.mockSuccessRate !== undefined
      ? { successRate: config.mockSuccessRate }
      : {}),
    ...(config.mockLatencyMs !== undefined
  ...
  const consumer = new KafkaConsumer(kafka, {
    ...
    topics: [commandTopic("email"), ...allRetryTopics("email")],
...
main().catch((err: unknown) => {
  console.error("services/worker-email failed to start:", err);
  process.exitCode = 1;
});
