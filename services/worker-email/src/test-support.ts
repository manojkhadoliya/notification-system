// In-memory fakes for every port `services/worker-email` depends on
...
export class FakeNotificationRepository implements NotificationRepository {
  ...
  async findById(): Promise<NotificationRequest | null> {
    return null; // not exercised by services/worker-email
  }

  async save(): Promise<void> {
    // not exercised by services/worker-email
  }

  async findAttempts(): Promise<DeliveryAttempt[]> {
    return []; // not exercised by services/worker-email
  }
